...
SystemD uses the journal log system for all logs. This means that you can access k0s and all the sub-component logs using `journalctl`. For example if you are interested in kubelet logs, you can run:

```shell
journalctl -u k0sworker | grep component=kubelet
```

### openRC based setups

openRC stores logs of services in `/var/log/k0sworker.log`. Again, if you're interested in specific component logs you can run:

```shell
grep component=kubelet /var/log/k0s.log
```
