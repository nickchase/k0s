...
k0s can be installed without a config file using the default configuration. You can, however, create and run your own non-default configuration to be used by the k0s controller nodes.

k0s supports providing only partial configurations, in which case k0s uses the defaults for any missing values. Follow these steps:
...
2. Modify the new yaml config file according to your needs, refer to [Configuration file reference](#configuration-file-reference) below. Feel free to remove any settings you don't need to change.
...
4. If you need to modify your existing configuration later on, you can change your config file and restart for the changes to take effect:
...
This is a YAML config file with defaults as generated by the `k0s config create` command:
...
| `onlyBindToAddress` | The API server binds to all interfaces by default. With this option set to `true`, the API server will only listen on the IP address configured by the `address` option (by default, the first non-local address by default). This can be necessary with multi-homed control plane nodes. |
...
k0s can also work with an externally managed etcd cluster. In this case, k0s will NOT set up etcd, it has to be managed manually.

| Element          | Description                                                                                                                                               |
|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| `endpoints`      | Array of etcd endpoints to use.                                                                                                                           |
| `etcdPrefix`     | Prefix to use for this cluster. The same external etcd cluster can be used for several k0s clusters, each prefixed with a unique prefix with which to store data. |
| `caFile`         | The host path to a file with the etcd cluster CA certificate.                                                                                       |
| `clientCertFile` | The host path to a file with the TLS certificate for the etcd client.                                                                           |
| `clientKeyFile`  | The host path to a file with TLS key for the etcd client.                                                                                    |

### `spec.network`

| Element         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|-----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `provider`      | The network provider. Valid values are `calico`, `kuberouter`, or `custom`. For `custom`, you can push any network provider (default: `kuberouter`). Be aware that it is your responsibility to configure all of the CNI-related setups, including the CNI provider itself and all necessary host level setups (for example, CNI binaries). **Note:** Once you initialize the cluster with a network provider the only way to change providers is through a full cluster redeployment. |
...
Environment variable values must be strings, as in:
...
K0s runs Calico with some predefined variables, which can be overwritten by setting a new value in `spec.network.calico.envVars`:
...
| `mtu`            | Override the MTU setting, if `autoMTU` must be set to `false`).                                                                                                                                                                                                               |
| `metricsPort`    | Kube-router metrics server port. The default value is `8080`. Set to 0 to disable metrics.                                                                                                                                                                                          |
...
| `ipMasq`         | IP masquerade for traffic originating from the pod network, and destined for outside of it (default: false)                                                                                                                                                                   |
...
| `type`           | The type of node-local load balancer to deploy on worker nodes. Default: `EnvoyProxy`. (This is the only option for now.) |
...
| `apiServerBindPort`          | Port number on which to bind the Envoy load balancer for the Kubernetes API server on a worker's loopback interface.  Default: `7443`. |
| `konnectivityServerBindPort` | Port number on which to bind the Envoy load balancer for the konnectivity server on a worker's loopback interface. Default: `7132`.    |
...
| Element          | Description                                                                                                 |
| ---------------- | ----------------------------------------------------------------------------------------------------------- |
| `vrrpInstances`  | Configuration options related to the VRRP. This is an array which allows k0s to configure multiple virtual IPs. |
| `virtualServers` | Configuration options related LoadBalancing. This is an array which allows k0s to configure multiple LoadBalancers.       |
...
| Element                 | Description                                                                                                                                                                                                                                                                                          |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `virtualIPs`            | The list of virtual IP address used by the VRRP instance. Each virtual IP must be a CIDR as defined in RFC 4632 and RFC 4291.                                                                                                         |
| `interface`             | The NIC used by the virtual router. If not specified, k0s will use the interface that owns the default route.                                                                                                                                                                                        |
| `virtualRouterID`       | The VRRP router ID. If not specified, k0s will automatically number the IDs for each VRRP instance, starting with 51. It must be in the range of 1-255, and all control plane nodes must use the same `virtualRouterID`. Other clusters in the same network must not use the same `virtualRouterID`. |
...
| -------- | -------------------------------------------------------------------------------- |
| `name`   | The name to use as profile selector for the worker process. Must be a string.                   |
| `values` | [Kubelet configuration][kubelet-config] overrides, see below for details. Must be an Object. |
...
k0s allows you to completely disable some of the system components. This allows
users to build a minimal Kubernetes control plane and use just the components
they need. Disabling the system components happens through a command line flag for the controller process:
...
used. It is scheduled for removal in k0s 1.27. Please update to the new
component name.

If you use k0sctl, just add the flag when installing the cluster for the first
controller at `spec.hosts.installFlags` in the config file, as in:
